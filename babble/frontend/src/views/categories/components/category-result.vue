  ...
  <el-row class="result-container">
    <el-col :offset="18">
      <div class="switch-order">
        <el-radio v-model="state.radio" label="best" @change="changetoBest"
          >인기순 정렬</el-radio
        >
        <el-radio v-model="state.radio" label="recent" @change="changetoRecent"
          >최신순 정렬</el-radio
        >
      </div>
    </el-col>
    <el-col>
      <el-row class="conference-row" v-if="state.radio == 'best'">
        <div v-if="state.bestRoomCount == 0">
          해당 카테고리의 대화중인 방이 없습니다. 만들어보세요!
        </div>
        <Conference
      ...
      <el-row class="conference-row" v-if="state.radio == 'recent'">
        <div v-if="state.recentRoomCount == 0">
          해당 카테고리의 대화중인 방이 없습니다. 만들어보세요!
        </div>
        <Conference
    ...
    <el-col :offset="20">
      <el-button
        type="text"
        @click="clickMore"
        v-if="state.bestRoomCount >= 10 || state.recentRoomCount >= 10"
        >더보기</el-button
      >
    </el-col>
  </el-row>
  <button v-show="state.showBacktop" class="backtop" @click="clickTop">
    Top
  </button>
  <ConferenceDialog
